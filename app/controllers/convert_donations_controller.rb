class ConvertDonationsController < ApplicationController
  ...
  def replay
    ...
    widget = catch_widget
    valid_donation = Donation.where('donations.email =  ? and donations.widget_id = ?', email, widget.id).order("created_at DESC").first

    if params[:utf8].present? && valid_donation.present? && valid_donation.transaction_status == 'paid' && Time.parse(valid_donation.created_at) > t
