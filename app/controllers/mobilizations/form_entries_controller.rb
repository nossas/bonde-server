  ...
  def create
    ...
    fields.each do |field|
      if !email && field['kind'] == 'email'
        email = field['value']
      elsif !first_name && ['nome'].include?(field['label'].downcase)
        first_name = field['value']
      elsif !last_name && ['sobrenome', 'sobre-nome', 'sobre nome'].include?(field['label'].downcase)
        last_name = field['value']
      elsif !phone && ['telefone', 'fone', 'celular'].include?(field['label'].downcase)
        phone = field['value']
      end
    end
    merge_vars = {}
