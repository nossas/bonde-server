class ChangeFunctionResetPasswordTokenRequestToGetCallbackUrl < ActiveRecord::Migration
  def up
    execute <<-SQL
      drop function if exists postgraphql.reset_password_token_request(email text, locale text);

      CREATE OR REPLACE FUNCTION postgraphql.reset_password_token_request(email text, callback_url text, locale text DEFAULT 'pt-BR'::text)
       RETURNS void
       LANGUAGE plpgsql
      AS $function$
          declare
